...
TEST_F(TOMLConfigBuilderTest, TestTOMLWithNestedTables) {
    ...
    const Config* child3_1 = children2[0] ;
    const Config* child3_2 = children2[1] ;
    ASSERT_FALSE(child3_1->isArray()) ;
    ASSERT_TRUE(child3_2->isArray()) ;
    ASSERT_EQ(child3_1->name(),"C") ;
    ASSERT_EQ(child3_2->name(),"X_Array") ;
    const ConfigArray* array = static_cast<const ConfigArray*>(child3_2) ;
    ASSERT_EQ(array->numElements(), 2) ;
    const Config* elem1 = array->at(0) ;
    const Config* elem2 = array->at(1) ;
    ASSERT_EQ(elem1->name(), "X") ;
    ASSERT_EQ(elem2->name(), "X") ;
}    
